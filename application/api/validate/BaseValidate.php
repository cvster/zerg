...
class BaseValidate extends Validate
{
    /**
     *
     * goCheck方法验证不通过后会抛出异常返回给客户端，check方法是TP5自带的方法，验证不通过后会返回false给函数调用者。
     *
    ...
    {
        $ruleMobile = '^1(3|4|5|7|8)[0-9]\d{8}$^';
        $rulePhone = '^0\d{2,3}-?\d{7,8}$^';
        $result = preg_match($ruleMobile, $value) || preg_match($rulePhone, $value) ;
        if ($result) {
            return true;
        } else {
            return false;
